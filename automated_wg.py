...
if server_public_keyfile.is_file() and server_private_keyfile.is_file():
    ...
    print('---Welcome to the AutomatedWireGuard-Script---\nIt will help you install a fully working WireGuard-Server.\nThe total number of existing clients is saved in /etc/wireguard/total_clients.txt and the variables of the first-time-installation are being saved in /etc/wireguard/variables.txt.\nIf you want to change any variables, just delete the variables.txt.\nNext time you want to create a new client, the script will ask you for each variable again.\nDetected first time installation. Installing Updates, WireGuard, generating keys and installing qrencode...')
